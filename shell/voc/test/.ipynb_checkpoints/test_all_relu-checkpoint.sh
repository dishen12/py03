...
weight_file=./weights/VOC/all_relu_512
eval_folder=./eval/VOC/all_relu_512
for((i=10;i<=400;i+=5));do
    weight=$weight_file/relu_mid_all_relu_VOC_epoches_$i.pth
    if [ ! -f "$weight" ]; then
    ...
    echo $weight
    echo $save_folder
    python test_RFB.py -d VOC -v relu_mid_all_relu -s 512 -r "9,5,2,1" -m  $weight --save_folder=$save_folder
done
