...
weight_file=./weights/VOC/relu_mid_rr_512
eval_folder=./eval/VOC/relu_mid_rr_512
for((i=10;i<=400;i+=5));do
    weight=$weight_file/relu_mid_rr_VOC_epoches_$i.pth
    if [ ! -f "$weight" ]; then
    ...
    echo $weight
    echo $save_folder
    python test_RFB.py -d VOC -v relu_mid_rr -s 512 -r "9,5,2,1" -m  $weight --save_folder=$save_folder
done
