...
parser.add_argument('-t', '--coco_test', default=False, type=bool,
                    help='the set for coco,test-dev or minval set')
...
print("num priors is !!!!!!!!!!!!!!!!!",num_priors)
...
def test_net(save_folder, net, detector, cuda, testset, transform, max_per_image=300, thresh=0.005):
    ...
    _t = {'im_detect': Timer(), 'misc': Timer(),'temp_misc':Timer()}
    ...
    for i in range(num_images):
        ...
        for j in range(1, num_classes):
            ...
            #keep = keep[:50]
            c_dets = c_dets[keep, :]
            #print("c_dets-----",c_dets.shape)
            all_boxes[j][i] = c_dets
            #print("iter time -----------------------",iter_time)
        nms_time = _t['misc'].toc()
        if max_per_image > 0:
            image_scores = np.hstack([all_boxes[j][i][:, -1] for j in range(1,num_classes)])
            #print("2---------------------------",image_scores.shape)
