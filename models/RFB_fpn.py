...
class Aspp_fpn(nn.Module):
    ...
    def __init__(self,in_planes,out_planes,stride=1,scale=0.1,rate=[6,3,2,1]):
        ...
        if(len(rate)==4):
            ...
            self.branch0_1 = BasicConv(2*inter_planes, 2*inter_planes, kernel_size=3, stride=1, padding=1, relu=False)
            self.branch0_2 = BasicConv(2*inter_planes, 2*inter_planes, kernel_size=3, stride=1, padding=1, relu=False)
            self.branch1 = nn.Sequential(
                    BasicConv(in_planes, 2*inter_planes, kernel_size=1, stride=stride),
                    BasicConv(2*inter_planes, 2*inter_planes, kernel_size=5, stride=1, padding=2, relu=False))
            self.branch1_1 = BasicConv(2*inter_planes, 2*inter_planes, kernel_size=5, stride=1, padding=2, relu=False)
            self.branch1_2 = BasicConv(2*inter_planes, 2*inter_planes, kernel_size=5, stride=1, padding=2, relu=False)  
            self.branch2 = nn.Sequential(
                    BasicConv(in_planes, 2*inter_planes, kernel_size=1, stride=stride),
                    BasicConv(2*inter_planes, 2*inter_planes, kernel_size=7, stride=1, padding=3, relu=False))
            self.branch2_1 = BasicConv(2*inter_planes, 2*inter_planes, kernel_size=7, stride=1, padding=3, relu=False)
            self.branch2_2 = BasicConv(2*inter_planes, 2*inter_planes, kernel_size=7, stride=1, padding=3, relu=False)
            self.branch3 = nn.Sequential(
                    BasicConv(in_planes, 2*inter_planes, kernel_size=1, stride=stride),
                    BasicConv(2*inter_planes, 2*inter_planes, kernel_size=9, stride=1, padding=4, relu=False))
            self.branch3_1 = BasicConv(2*inter_planes, 2*inter_planes, kernel_size=9, stride=1, padding=4, relu=False)
            self.branch3_2 = BasicConv(2*inter_planes, 2*inter_planes, kernel_size=9, stride=1, padding=4, relu=False)
...
class RFBNet(nn.Module):
    ...
    def __init__(self, phase, size, base, extras, head, num_classes):
        ...
        self.Norm = Aspp_fpn(512,512,stride=1,scale=1)
    ...
    def forward(self, x):
        ...
        #print("the size of loc is ",len(loc))
        ...
        if in_channels != 'S':
            if v == 'S':
                if in_channels == 256 and size == 512:
                    layers += [Aspp_fpn(in_channels,cfg[k+1],stride=2,scale=1,rate=Rate)]
                else:
                    layers += [Aspp_fpn(in_channels,cfg[k+1],stride=2,scale=1,rate=Rate)]
            else:
                layers += [Aspp_fpn(in_channels,v,scale=1,rate=Rate)]
        in_channels = v
    if size == 512:
